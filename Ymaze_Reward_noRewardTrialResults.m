 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  ...
  lickDuration = 5;  % Reward_Time ± lickDuration (3s etc) is considered to show lick occurrences 
  selectedDuration =10 ;  % This duration is considered to check lickrate 
  ...
  for i=1:noOfSelectedFiles  %  Loop through each selected file
      ...
      [lickCountRewardArrayAllTrials, aveLickCountRewardArray,rewardLick_SEM]= findAverageLickRate(rewardLickArray,lickDuration,selectedDuration);
      [lickCountNoRewardArraysAllTrials, aveLickCountNoRewardArray,noRewardLick_SEM]= findAverageLickRate(noRewardLickArray,lickDuration,selectedDuration);
      ...
      writetable(output3DataTable5,fullfile(filePath,'Results/',joinedFileName,'/',outputFileName3),'sheet', strcat(sessionNames{i},'- Reward Trials') ,'Range','A1') ;
      lickCountRewardArrayAllTrials = [transpose(lickCountRewardArrayAllTrials)]; 
      lickCountRewardArrayAllTrials = array2table(lickCountRewardArrayAllTrials, 'VariableNames' ,strcat('Reward_Trial_NO_',string(Index_Reward(1:length(Index_Reward)))))  ;
      writetable(lickCountRewardArrayAllTrials,fullfile(filePath,'Results/',joinedFileName,'/',outputFileName3),'sheet', strcat(sessionNames{i},'- Reward Trials') ,'Range','B1');  
      ...
      writetable(output3DataTable5,fullfile(filePath,'Results/',joinedFileName,'/',outputFileName3),'sheet', strcat(sessionNames{i},'- NoReward Trials') ,'Range','A1') ;
      lickCountNoRewardArraysAllTrials = [transpose(lickCountNoRewardArraysAllTrials)]; 
      lickCountNoRewardArraysAllTrials = array2table(lickCountNoRewardArraysAllTrials, 'VariableNames' ,strcat('NoReward_Trial_NO_',string(Index_noReward(1:length(Index_noReward)))))  ;
      writetable(lickCountNoRewardArraysAllTrials,fullfile(filePath,'Results/',joinedFileName,'/',outputFileName3),'sheet', strcat(sessionNames{i},'- NoReward Trials') ,'Range','B1');  
...
function [lickCountArray, aveLickCountArray, SEM_Array]=  findAverageLickRate(lickTimeArray,lickDuration,selectedDuration)
   % This function returns Average lick Rate for each time window of the selected period
